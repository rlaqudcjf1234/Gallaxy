...
public class LoginRegiFrame extends JFrame {
	private LoginUserDataSet users;
	...
	public LoginRegiFrame() {
		...
		btnSignup.setBounds(129, 550, 100, 30); // 위치 및 크기 설정
		signupPanel.add(btnSignup);

		btnCancel = new JButton("취소");
		btnCancel.setBounds(259, 550, 100, 30); // 위치 및 크기 설정
		...
		btnSignup.addActionListener(new ActionListener() {
		    ...
		    public void actionPerformed(ActionEvent e) {
		        ...
		        if (tfId.getText().isEmpty() || tfPw.getPassword().length == 0 ||
		            ...
		            JOptionPane.showMessageDialog(null, "비밀번호가 일치하지 않습니다.", "입력 오류", JOptionPane.ERROR_MESSAGE);
		        } else {
		            // 성공적인 회원가입 처리
		            String gender = rbtnMale.isSelected() ? "남성" : "여성";
		            JOptionPane.showMessageDialog(null, "회원가입 성공!\n" +
		                    "ID: " + tfId.getText() + "\n" +
		                    "이름: " + tfName.getText() + "\n" +
		                    "이메일: " + tfEmail.getText() + "\n" +
		                    "별명: " + tfNickName.getText() + "\n" +
		                    "성별: " + gender, "회원가입 완료", JOptionPane.INFORMATION_MESSAGE);
		            // 입력 필드 초기화
		            resetFields();

		            // 새 화면으로 이동
		            new LoginFrame(); // MainScreen 인스턴스 생성
	...
	private void resetFields() {
		...
		tfEmail.setText("");
		tfNickName.setText("");
		rbtnMale.setSelected(true); // 기본값으로 남성 선택
	}

	public static void main(String[] args) {
		SwingUtilities.invokeLater(LoginRegiFrame::new); // UI 생성
	}
}